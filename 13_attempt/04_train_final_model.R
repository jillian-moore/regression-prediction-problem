...
# load data ----
load(here("13_attempt/data_split/asheville_test.rda"))
load(here("13_attempt/data_split/chicago_test.rda"))  
load(here("13_attempt/data_split/kauai_test.rda"))
...
# load tuning results ----
load(here("13_attempt/results/13a_bt_combined_results.rda"))
...
train_location_model <- function(location_name, location_config) {
  ...
  save(
    ...
    best_params,
    predictions,
    file = here("13_attempt/results", paste0("13a_", location_name, "_final_model.rda"))
  )
...
# save out ----
write_csv(final_submit, here('13_attempt/results/13a_bt_submission.csv'))
write_csv(combined_predictions, here('13_attempt/results/13a_bt_predictions_with_locations.csv'))
write_csv(model_summary, here('13_attempt/results/13a_model_summary.csv'))
save(
  final_results,
  combined_predictions,
  final_submit,
  model_summary,
  file = here("13_attempt/results/13a_final_results.rda")
)